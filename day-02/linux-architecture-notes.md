...
-  Uninterruptible sleep (waiting for disk I/O, cannot be killed easily)
...
### Command 1 --- List your processes / active processes / only your active terminal

    ps

### Command 2 --- List all system processes (Shows all processes in system (user + system))

    ps aux

### Command 3 --- Processes from all terminals (no system processes)

    ps a
    
### Command 4 --- Real-time process monitoring
...
1. Sends **SIGTERM** (signal 15)
2. Process gets chance to clean up
3. Preferred way
...
**Force kill:**
    ...
    kill -9 <PID>
1. Sends **SIGKILL** (signal 9)
2. Immediate termination
3. No cleanup, no warning

**Find and kill in one command:**

    pkill python
1. Kills processes by name
2. By default → sends SIGTERM
3. It kills ALL processes with that name
...
-   It starts and manages background services like:
    ...
    -   docker
    -   mysql
-  Daemon = background process/service in Linux
-  Runs continuously without user interaction
-  Usually starts at boot and keeps running

## 6) systemd -- Most Important Commands (Daily Use)

Check service status:

    systemctl status ssh                # Is the service running? Any errors?
